// Variable global para seguir el modo de pintura actual
let modoActual = 'vinilica';
...
function inicializarFormularioCarga() {
    ...
    // Asigna eventos a los botones para cambiar de modo
    const modeVinilica = document.getElementById('modeVinilica');
    const modeEsmalte = document.getElementById('modeEsmalte');

    modeVinilica.addEventListener('click', () => {
        modoActual = 'vinilica';
    });

    modeEsmalte.addEventListener('click', () => {
        modoActual = 'esmalte';
    });
    
    async function actualizarEnvasesDisponibles(codigo) {
        // Lógica actualizada para el modo de esmalte
        if (modoActual === 'esmalte') {
            // Si es pintura de esmalte, no necesitamos validar con la API.
            // Simplemente mostramos todos los campos para que el usuario pueda ingresar lo que quiera.
            ...
            return; // Salimos de la función para no ejecutar el código de la API.
        }
        ...
        try {
           const response = await fetch(`https://pintumexplaneacion-1.onrender.com/api/productos/envases/${codigo}`);
            ...
            if (disponibles.length === 0) { 
                mostrarNotificacion('No hay envases disponibles para este código', 'error'); 
            }
        } catch (error) {
...
function inicializarExportacionOrdenes() {
    ...
    inputPdf.addEventListener('change', async (event) => {
        ...
        try {
            const res = await fetch("https://subrayadoordenes-5.onrender.com/procesar_pdf", {
    method: "POST",
    body: formData
});
