...
class EsmalteMenuManager {
    ...
    async cargarDatosEsmalte() {
        ...
        try {
            // Nota: Es posible que necesites ajustar esta URL para obtener los datos correctos.
            // La URL 'http://localhost:3000/api/esmaltes' debería devolver el mismo formato
            // que tu otra función `asignarCargasEsmalte` genera.
    ...
    renderizarCargasEsmalte(cargas) {
        ...
        cargas.forEach(carga => {
            // CORRECCIÓN APLICADA AQUÍ: usar 'asignadoMolienda' y 'asignadoTerminado'
            const personalAsignado = [carga.asignadoMolienda, carga.asignadoTerminado].filter(Boolean).join(', ');
            html += `
    ...
    renderizarResumenPersonal(cargas) {
        ...
        cargas.forEach(carga => {
            // CORRECCIÓN APLICADA AQUÍ: usar 'asignadoMolienda' y 'asignadoTerminado'
            const moliendaPreparado = carga.asignadoMolienda;
            const terminado = carga.asignadoTerminado;

            if (moliendaPreparado) {
                resumen['Molienda y Preparado'][moliendaPreparado] = (resumen['Molienda y Preparado'][moliendaPreparado] || 0) + 1;
            }
            if (terminado) {
