    ...
    for (const maquina of maquinas) {
        ...
        for (let ronda = 1; ronda <= rondasTotales; ronda++) {
            ...
            cargas.forEach(c => {
                ...
                if (c.cubetas > 0) {
                    detailsArray.push(`19 - ${(c.cubetas * 19).toFixed(0)}L`);
                }
                if (c.galones > 0) {
                    detailsArray.push(`4 - ${(c.galones * 4).toFixed(0)}L`);
                }
                if (c.litros_envase > 0) { // Using litros_envase for 1L liters
                    detailsArray.push(`1 - ${c.litros_envase}L`);
                }
                if (c.medios > 0) {
                    detailsArray.push(`0.5 - ${(c.medios * 0.5).toFixed(0)}L`);
                }
